...
class Input:
	...
	def serialize(self):
		d = { 'where_created': self.where_created,
		      'value': self.value,
...
class UnsignedTransaction:
	def __init__(self, txType, input_coins, output_coins, txID=None):
		...
		else: self.txID = self.generate_txID() 
...
class Transaction:
	def __init__(self, unsigned, signatures):
		...
		self.signatures = signatures # raw signatures (as a ECC signature object); stored in a  dictionary ['str(sig)'] = sig
#		self.scroge_signature = Scrooge()

	#Better have Scrooge sign a block, and the transaction can be block's data
#	def Scrooge(self):
#		return True
